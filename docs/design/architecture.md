...
│  │  - 통합 인터페이스 (CLI/웹 UI)            │        │
│  │  - 인자 검증 및 파싱                      │        │
│  │  - Main Agent 호출 (Phase 1-3.5)         │        │
│  │  - /draftify-ppt 호출 (Phase 4)          │        │
│  └──────────────────────────────────────────┘        │
└─────────────┬───────────────────────┬────────────────┘
              │                       │
              │ Task tool 호출         │ Phase 4: Skill 호출
              ▼                       ▼
┌──────────────────────────────────────────────────────┐
│               오케스트레이션 계층                      │
│  ┌──────────────────────────────────────────┐        │
│  │  Main Agent: auto-draft-orchestrator     │        │
│  │  - Phase 1-3.5 워크플로우 제어           │        │
│  │  - 서브 에이전트 생명주기 관리             │        │
│  │  - 에러 핸들링                            │        │
│  │  - 재시도 전략                            │        │
│  │  - 결과 반환 → 스킬 계층 (Phase 4)        │        │
...
#### 3. 스킬 계층 → 스킬 (/auto-draft → /draftify-ppt)

**draftify-ppt 스킬** (Phase 4):
- 별도 독립 스킬
- 마크다운 섹션 → PPT 변환
- **/auto-draft 스킬 계층**에서 호출 (orchestrator 아님)

> **Note**: 서브 에이전트는 스킬을 호출할 수 없으므로, Phase 4는 /auto-draft 스킬이 orchestrator 결과를 받은 후 직접 호출합니다.
...
| **스킬 계층** | 인자 검증, orchestrator 호출, Phase 4 실행 | Task tool, Skill tool | 메인 컨텍스트 |
| **오케스트레이션 계층** | Phase 1-3.5 제어, 에러 핸들링 | Task, Bash, Read/Write | 독립 컨텍스트 (25분) |
